...
generateFeatureSelectionMethods <- function(searchMethods,
  ...
  colnames(combinations) <- c("searchMethod", "multivariateCriterion")
  
  combinations <- arrange(combinations, searchMethod)
  
  featureSelectionMethods <- apply(
    combinations, 1,
    function(r) {
      
      searchMethod <- searchMethods[[r["searchMethod"]]]
      multivariateCriterion <- multivariateCriterions[[r["multivariateCriterion"]]]
      
      featureSelectionMethod <- generateFeatureSelectionMethod(
        searchMethod, multivariateCriterion, ...)
      
      return(featureSelectionMethod)
    })
  
  names(featureSelectionMethods) <- apply(
    combinations, 1, 
    function(r) {
