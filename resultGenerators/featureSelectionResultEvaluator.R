...
selectFeaturesWithCriterionAndAssess <- function(multivariateCriterion,
                                                 searchMethod,
                                                 dataset,
  ...
  featureSelectionMethod <- function(trainData, trainLabels) {
    
    selectFeaturesWithCriterion(searchMethod = searchMethod,
  ...
  computeFeatureSelectionResult(dataset,
...
computeFeatureSelectionResult <- function(dataset,
  ...
  extractFeatureSelectionResultFrom <- function(crossValidationResult) {
    ...
    for (classifierName in names(assessmentClassifiers)) {
      ...
      classifierPerformanceInfo <- lapply(crossValidationResult,
                                          function (foldResult) {
  ...
  folds <- cvFolds(n = nObservations, K = nFolds, type = "interleaved")
  ...
  featureSelectionResult <-
    extractFeatureSelectionResultFrom(crossValidationResult)
