...
computeHierarchicalClusteringFor <-
  function(selectedFeatureSubsets) {
    
    getSelectedFeaturesForMethod <- 
      function(methodName) {
    ...
    apply(methodsCombinations, 2,
          function (combination) {
            ...
            distanceMatrix[method2, method1] <<-
              jaccardDistance(
...
jaccardDistance <- function(selectedSubsets1, selectedSubsets2) {
  ...
  jaccardDistancesForDatasets <-
    mapply(meanJaccardDistanceForDatasetResult,
           selectedSubsets1,
           selectedSubsets2)
...
meanJaccardDistanceForDatasetResult <- function(subsets1, subsets2) {
  ...
  jaccardDistances <- 
    mapply(featureSubsetJaccardDistance,
           subsets1, subsets2)
