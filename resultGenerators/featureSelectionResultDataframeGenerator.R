...
featureSelectionResultDataframeGenerator <- function(featureSelectionMethodsResultInfo,
                                                     assessmentClassifiers) {
  
  ## Project all selected features subsets.
  allSelectedFeatures <- lapply(
  ...
  individualClassifiersPerformances <- t(sapply(
    featureSelectionMethodsResultInfo,
    function (resultInfo) {
      checkInstall("gdata")
  ...
  overallClassifiersPerformances <- t(sapply(
    featureSelectionMethodsResultInfo, 
    function (resultInfo) {
  ...
  encodedSelectedFeatures <- sapply(
    allSelectedFeatures, encodeFeatureSubsetList)
  
  ## construct the data.frame wrapping the computed metrics
  resultDataFrame <- data.frame(
    featureSelectionMethods = rownames(overallClassifiersPerformances),
    sizeMetricsMatrix,
