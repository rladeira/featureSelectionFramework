...
barplotForElapsedMinutes <- function (result) {
  ...
  data$featureSelectionMethods <-
    ordered(data$featureSelectionMethod,
...
boxplotsForAllMetrics <- function (result) {
  ...
  featureSelectionMethodNames <- sort(as.character(result[[1]]$combined$featureSelectionMethod))
  
  for (metric in metrics) {
    
    metricDataFrame <-
      sapply(result[[1]]$datasetsResults,
    ...
    fsOrdering <- order(as.numeric(by(meltedMetrics$values,
                                      meltedMetrics$featureSelectionMethods,
                                      median)))   
    
    meltedMetrics$featureSelectionMethods <-
      ordered(meltedMetrics$featureSelectionMethods,
